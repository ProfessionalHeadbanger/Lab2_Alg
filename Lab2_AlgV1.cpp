...
int* Merge(int** A, int* first, int* second, int first_size, int second_size, int lines_count, int i = 0)
{
    int curr_line = i;
    int index1 = 0;
    int index2 = 0;
    ...
    if (curr_line + 1 < lines_count - 1)
    {
        curr_line++;
        Merge(A, result, A[curr_line + 1], result_size, second_size, lines_count, curr_line);
    }
    else
    {
        return result;
...
int main()
{
    ...
    int* result = Merge(A, A[0], A[1], m, m, n);
